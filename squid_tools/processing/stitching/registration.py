...
def find_adjacent_pairs(tile_positions, pixel_size, tile_shape, min_overlap=15):
    ...
    for i_pos in range(n_pos):
        for j_pos in range(i_pos + 1, n_pos):
            ...
            overlap_y = tile_h - abs(dy)
            overlap_x = tile_w - abs(dx)
...
def compute_pair_bounds(adjacent_pairs, tile_shape):
    ...
    for i_pos, j_pos, dy, dx, _overlap_y, _overlap_x in adjacent_pairs:
        bounds_i_y = (max(0, dy), min(tile_h, tile_h + dy))
        bounds_i_x = (max(0, dx), min(tile_w, tile_w + dx))
        bounds_j_y = (max(0, -dy), min(tile_h, tile_h - dy))
        bounds_j_x = (max(0, -dx), min(tile_w, tile_w - dx))
